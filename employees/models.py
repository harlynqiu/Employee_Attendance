...
class Employee(models.Model):
    ...
    pagibig_file = models.FileField(
        upload_to="government_documents/pagibig/",
        blank=True,
        null=True,
    )
    ...
    # Existing optional government fields are preserved for compatibility.
    ...
    def qr_contact_number(self):
        """
        Use the company contact number when configured.

        Otherwise, use the employee's contact number.
        """
    ...
    def qr_data(self):
        ...
        """
        position = (self.position or "").strip().upper()
    ...
    def clean(self):
        ...
        if self.rate is not None and self.rate <= 0:
            errors["rate"] = (
                "Salary rate must be greater than 0."
            )
    ...
    def get_next_employee_id(cls):
        """
        Return the next available HCQ number.

        Malformed older employee IDs are ignored.
        """
        highest_number = 0
    ...
    def qr_details_changed(self):
        ...
        old_values = type(self).objects.filter(
            pk=self.pk,
        ).values(
            "employee_id",
            "first_name",
    ...
    def save(self, *args, **kwargs):
        if not self.employee_id:
            self.employee_id = (
                self.get_next_employee_id()
            )

        should_regenerate_qr = (
            not self.qr_code
            or self.qr_details_changed()
        )
